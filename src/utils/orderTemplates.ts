...
export const ORDER_TEMPLATES: Record<string, OrderTemplate> = {
  'create_port': {
    ...
    fields: [
      ...
      { name: 'nonMobileMain', mandatory: true, type: 'string', example: '9000111110', description: 'Non-mobile main number' },
      ...
      { name: 'orderRef', mandatory: false, type: 'string', example: 'AWN-NW-2024-000001', description: 'Order reference from SSS' },
    ]
  },
  'terminate_port': {
    type: 'terminate_port',
  ...
  'create_lag': {
    ...
    fields: [
      ...
      { name: 'caNo', mandatory: true, type: 'string', example: 'C230065571', description: 'Customer account no.' },
      { name: 'customerAccountName', mandatory: true, type: 'string', example: 'Test Customer Account', description: 'Customer account name' },
      { name: 'customerType', mandatory: true, type: 'string', example: 'Enterprise', description: 'Customer type' },
      { name: 'cableType', mandatory: true, type: 'string', example: 'Main', description: 'Cable type: Main or Backup' },
      { name: 'customerSiteCode', mandatory: true, type: 'string', example: 'CX_123456', description: 'Customer site code' },
      { name: 'dcLocation', mandatory: true, type: 'string', example: 'STTT3', description: 'Data Center Location' },
      ...
      { name: 'reservedPortId', mandatory: true, type: 'string', example: 'PORT-LAG-12345', description: 'Reserved port ID from inventory' },
      { name: 'nonMobileNo', mandatory: false, type: 'string', example: '9000111111', description: "Customer's non-mobile number" },
      ...
      { name: 'serviceVlanType', mandatory: false, type: 'string', example: 'dot1q', description: 'Service VLAN type: dot1q, access, qinq' },
      { name: 'orderRef', mandatory: false, type: 'string', example: 'AWN-NW-2024-000002', description: 'Order reference from SSS' },
    ]
  },
  'terminate_lag': {
    type: 'terminate_lag',
  ...
  'create_vxc': {
    ...
    fields: [
      ...
      { name: 'implementServiceOver', mandatory: true, type: 'string', example: 'Port', description: 'Implement service over: Port or LAG' },
      { name: 'nonMobileImplementServiceOver', mandatory: true, type: 'string', example: '9000111112', description: 'Non-mobile of service to implement over' },
      { name: 'networkServiceType', mandatory: true, type: 'string', example: 'L2VPN', description: 'Network service type' },
      { name: 'orderRef', mandatory: true, type: 'string', example: 'AWN-NW-2024-000003', description: 'Order reference from SSS' },
      { name: 'orderDate', mandatory: true, type: 'string', example: '17-11-2025', description: 'Order date (DD-MM-YYYY)' },
      { name: 'inventoryId', mandatory: true, type: 'string', example: 'INV-VXC-12345', description: 'Inventory ID (system-generated if empty)' },
      { name: 'transactionId', mandatory: true, type: 'string', example: generateUUID(), description: 'Transaction ID' },
      { name: 'processStatus', mandatory: true, type: 'string', example: 'InProgress', description: 'Process status' },
      { name: 'rack', mandatory: true, type: 'string', example: 'RACK-01', description: 'Rack location' },
      { name: 'lagNo', mandatory: true, type: 'string', example: '', description: 'LAG number (if implementing over LAG)' },
      { name: 'reservedPortId', mandatory: true, type: 'string', example: 'PORT-VXC-12345', description: 'Reserved port ID' },
      { name: 'vlanId', mandatory: false, type: 'string', example: '100', description: 'VLAN ID for VXC' },
    ]
  },
  'terminate_vxc': {
    type: 'terminate_vxc',
  ...
  'create_vxc_cloud': {
    ...
    fields: [
      ...
      { name: 'serviceVlanType', mandatory: true, type: 'string', example: 'dot1q', description: 'Service VLAN type: dot1q, access, qinq' },
      { name: 'implementServiceOver', mandatory: true, type: 'string', example: 'Port', description: 'Implement service over: Port or LAG' },
      { name: 'nonMobileImplementServiceOver', mandatory: true, type: 'string', example: '9000111113', description: 'Non-mobile of service to implement over' },
      { name: 'networkServiceType', mandatory: true, type: 'string', example: 'L2VPN', description: 'Network service type' },
      { name: 'cloudProvider', mandatory: true, type: 'string', example: 'AWS', description: 'Cloud provider (AWS, Azure, GCP)' },
      { name: 'bustLinkSpeed', mandatory: true, type: 'string', example: '1024 Mbps', description: 'Burst link speed [Mbps, Kbps, Gbps]' },
      { name: 'burstEffectiveDate', mandatory: true, type: 'string', example: '17-11-2025', description: 'Burst effective date (DD-MM-YYYY)' },
      { name: 'burstEndDate', mandatory: true, type: 'string', example: '31-12-2999', description: 'Burst end date (DD-MM-YYYY)' },
      { name: 'orderRef', mandatory: true, type: 'string', example: 'AWN-NW-2024-000004', description: 'Order reference from SSS' },
      { name: 'rack', mandatory: true, type: 'string', example: 'RACK-01', description: 'Rack location' },
      { name: 'lagNo', mandatory: true, type: 'string', example: '', description: 'LAG number (if implementing over LAG)' },
      { name: 'reservedPortId', mandatory: true, type: 'string', example: 'PORT-CLOUD-12345', description: 'Reserved port ID' },
      { name: 'vlanId', mandatory: false, type: 'string', example: '200', description: 'VLAN ID for cloud VXC' },
    ]
  },
  'terminate_vxc_cloud': {
